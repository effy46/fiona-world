...
// Where the character walks BEFORE the scene crossfades to a new section.
// Each scene's "exit" — the visual moment that justifies the transition.
const exitWaypointId: Record<SceneId, string> = {
  entry: 'entry-walkway-end',          // tower walkway — leads to bridge / dial
  projects: 'projects-return-door',
  skills: 'skills-return-door',
  thoughts: 'thoughts-return-door',
  contact: 'contact-return-door'
};
...
export function InteractiveWorld({ activeSection }: InteractiveWorldProps) {
  ...
  const pendingNavigation = usePortfolioStore((state) => state.pendingNavigation);
  const clearPendingNavigation = usePortfolioStore((state) => state.clearPendingNavigation);
  ...
  // Cinematic nav: when Header (or any caller) sets pendingNavigation,
  // walk Fiona to current scene's exit waypoint, then crossfade to target.
  useEffect(() => {
    if (!pendingNavigation) return;
    if (pendingNavigation === activeSection) {
      clearPendingNavigation();
      return;
    }
    let cancelled = false;
    (async () => {
      // Step 1: walk to exit waypoint of current scene
      const exitId = exitWaypointId[activeSection];
      const exitWp = sceneGraph.waypoints.find((point) => point.id === exitId);
      if (exitWp) {
        setCharacterFacing(activeSection === 'entry' ? 'bridge' : 'front');
        moveCharacter({ waypointId: exitWp.id, x: exitWp.x, y: exitWp.y });
        void playStep(audioEnabled);
        await new Promise((resolve) => window.setTimeout(resolve, 620));
      }
      if (cancelled) return;
      // Step 2: trigger scene crossfade + URL nav
      const target = pendingNavigation;
      const entrance = sceneGraph.waypoints.find((point) => point.id === entranceWaypointId[target]);
      const targetLabel = portfolioSections.find((item) => item.id === target)?.label ?? target;
      setWorldTransition({ key: Date.now(), from: activeSection, to: target, label: targetLabel });
      setCharacterFacing(target === 'entry' ? 'front' : 'bridge');
      if (entrance) moveCharacter({ waypointId: entrance.id, x: entrance.x, y: entrance.y });
      void playStep(audioEnabled);
      window.setTimeout(() => navigate(routeForScene(target)), 720);
      window.setTimeout(() => {
    ...
    <section className={`world-shell world-${activeSection}`} aria-labelledby="scene-title">
      <div className="scene-stage">
        <div className="scene-canvas">
          {activeSection === 'entry' ? (
            // Pre-rendered walking animation. NO live sprite overlay — character
            // walk is fully baked into the WebP. See scripts/generate-entry-walk.py.
            <img
              className="scene-image"
              src={`${base}entry-walk.webp`}
              alt="Fiona walks from the Entry Tower across the bridge"
